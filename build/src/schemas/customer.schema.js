"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getUserSchema = exports.updateCustomerSchema = exports.CreateCustomerSchema = void 0;
const zod_1 = require("zod");
const id = zod_1.z.string().uuid();
const name = zod_1.z.string().min(3).max(20);
const lastName = zod_1.z.string().min(4).max(20);
const userId = zod_1.z.string().uuid();
const email = zod_1.z.string().email();
const password = zod_1.z.string().min(4).max(10);
exports.CreateCustomerSchema = zod_1.z.object({
    body: zod_1.z.object({
        name: name.min(1),
        lastName: lastName.min(1),
        // userId: userId.min(1),
        user: zod_1.z.object({
            email: email.min(1),
            password: password.min(1),
        }),
    }),
});
exports.updateCustomerSchema = zod_1.z.object({
    body: zod_1.z.object({
        name: name.optional(),
        lastName: lastName.optional(),
        userId: userId.optional(),
    }),
    params: zod_1.z.object({
        id: id.min(1),
    }),
});
exports.getUserSchema = zod_1.z.object({
    params: zod_1.z.object({
        id: id.min(1),
    }),
});
//# sourceMappingURL=customer.schema.js.map